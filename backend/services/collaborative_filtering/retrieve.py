...
def retrieve(user_id: int):
    ...
    # df_ratings.duplicated(['userId', 'storeId'], keep='first')
    ...
    U, sigma, Vt = svds(matrix_user_mean, k=12) # k = hyperparameter
    # print(U.shape)
    # print(Vt.shape)
    # print(sigma.shape)
    # print(df_user_store_ratings.shape)
    # print(df_user_store_ratings.columns)

    sigma = np.diag(sigma)
    svd_user_predicted_ratings = np.dot(np.dot(U, sigma), Vt) + user_ratings_mean.reshape(-1,1)
    # print(np.dot(np.dot(U, sigma), Vt).shape)
    # print( user_ratings_mean.reshape(-1,1).shape)
    # print(svd_user_predicted_ratings.shape)
    ...
    recommendations = recommendations.rename(columns={user_row_number: 'Predictions'}).sort_values('Predictions', ascending=False).iloc[:10, :] # 10 -> recommendation number
